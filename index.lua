					...
					dbcon:free_disks_space(function(free_disks_space)
						dbcon:network_bytes_received_sent_persec(function(network_bytes)
							dbcon:connections_in_out(function(connections_in_out)
								p(string.format("PERCENT_PROCESSOR_TIME %s %s", percent_processor_time, source))
								p(string.format("PERCENT_DISKS_TIME %s %s", percent_disks_time, source))
								p(string.format("ALLOCATION_AVAILABLE_BYTES %s %s", allocation_available_bytes, source))
								p(string.format("MEMORY_SWAP_RATE %s %s", memory_swap_rate, source))
								p(string.format("FREE_DISKS_SPACE %s %s", free_disks_space, source))
								p(string.format("NETWORK_BYTES_RECEIVED_PERSEC %s %s", network_bytes["received"], source))
								p(string.format("NETWORK_BYTES_SENT_PERSEC %s %s", network_bytes["sent"], source))
								p(string.format("CONNECTIONS_ESTABLISHED %s %s", connections_in_out, source))
							end)
						end)
					end)
				end)
